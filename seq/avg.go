...
func MovingAvg[V types.Number](input iter.Seq[V], size int) iter.Seq[V] {
	return func(yield func(V) bool) {
		for items := range Window(input, size) {
			if !yield(cont.Sum(items) / V(len(items))) {
				return
			}
		}
	}
...
func CenteredMovingAvg[V types.Number](input iter.Seq[V], size int) iter.Seq[V] {
	return func(yield func(V) bool) {
		cache := []V{}
		pass := 0
		...
		for num := range input {
			...
			if !yield(cont.Sum(cache) / V(len(cache))) {
				return
			}
		}

		for idx := range pass {
			if !yield(cont.Sum(cache[idx:]) / V(len(cache)-idx)) {
				return
			}
		}
	}
