...
func Window2[K, V any](input iter.Seq2[K, V], size int) iter.Seq[[]types.Tuple[K, V]] {
	return func(yield func([]types.Tuple[K, V]) bool) {
		cache := []types.Tuple[K, V]{}

		for key, val := range input {
			cache = append(cache, types.T(key, val))
