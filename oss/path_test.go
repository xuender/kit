...
func TestAbs(t *testing.T) {
	...
	ass.Equal("parent", filepath.Base(ops.Must(oss.Abs("parent/c/.."))))
	ass.NotEqual("~", filepath.Base(ops.Must(oss.Abs("~"))))

	home := ops.Must(os.UserHomeDir())
	path := ops.Must(oss.Abs("~"))
	ass.Equal(home, path)

	path = ops.Must(oss.Abs("~/file"))
...
func TestAbs2(t *testing.T) {
	...
	path := ops.Must(oss.Abs("./ff"))
	pwd := ops.Must(os.Getwd())
