...
func TestRemove(t *testing.T) {
	...
	req := require.New(t)
	worker := "aa"
	dir := filepath.Join(os.TempDir(), worker)
	path := filepath.Join(dir, worker, worker, worker)

	req.NoError(os.MkdirAll(path, oss.DefaultDirFileMod))
	req.NoError(os.WriteFile(filepath.Join(path, worker), []byte("test"), oss.DefaultFileMode))
	req.NoError(os.WriteFile(filepath.Join(path, worker), []byte("test"), oss.DefaultFileMode))
	req.NoError(os.WriteFile(filepath.Join(path, worker), []byte("test"), oss.DefaultFileMode))
	req.NoError(oss.Remove(path, 3))
	ass.False(oss.Exist(dir))
	req.NoError(oss.Remove(filepath.Join(os.TempDir(), worker), 10))
}
...
func TestRemove2(t *testing.T) {
	...
	req.NoError(os.MkdirAll(path, oss.DefaultDirFileMod))
	req.NoError(os.WriteFile(filepath.Join(path, worker), []byte("test"), oss.DefaultFileMode))
	req.NoError(os.WriteFile(filepath.Join(filepath.Dir(path), "aa"), []byte("test"), oss.DefaultFileMode))
...
func TestRemove3(t *testing.T) {
	...
	req.NoError(os.MkdirAll(path, oss.DefaultDirFileMod))
	file := filepath.Join(path, worker)
